...
function musiccontrol.controls()
	...
	if (Controller.isButtonDown(0, "A") and Sound.isPlaying(soundIndex1) == false) then
		print("Play Startup.wav")
		Sound.play(soundIndex1)
	end 
	
	if (Controller.isButtonDown(0, "B") and Sound.isPlaying(soundIndex2) == false) then
		print("Play Intro.wav")
		Sound.play(soundIndex2)
	end

	if (Controller.isButtonDown(0, "X")) then
	...
	if (Controller.isButtonDown(0, "Y")) then
